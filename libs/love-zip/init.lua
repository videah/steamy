...
local nativefs = require(libpath..".nativefs")
...
local function writeCenteralDirectory(zipFile,filesInfos)
    ...
    return {
        offset = centeralDirectoryOffset or 0,
...
function zapi.createZip(path,zipFile)
    ...
    local function index(dir)
        ...
        if not dirInfo then print(dir) end
    ...
    if not nativefs.getInfo(path) then return false, "Source doesn't exist !" end
